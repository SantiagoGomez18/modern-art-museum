...
export class MuseumArtworkService {
  ...
  async addArtworkMuseum(
    ...
    const artwork: ArtworkEntity | null = await this.artworkRepository.findOne({
      where: { id: artworkId },
    });
    if (!artwork)
  ...
  async associateArtworksMuseum(museumId: string, artworks: ArtworkEntity[]): Promise<MuseumEntity> {
    ...
    for (let i = 0; i < artworks.length; i++) {
      const artwork: ArtworkEntity | null =
  ...
  async deleteArtworkMuseum(museumId: string, artworkId: string) {
    ...
    const museumArtwork: ArtworkEntity | undefined = museum.artworks.find(
      (e) => e.id === artwork.id,
    );
