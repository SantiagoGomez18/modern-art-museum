...
describe('MuseumArtworkService', () => {
  ...
  it('deleteArtworkToMuseum should remove an artwork from a museum', async () => {
    ...
    const storedMuseum: MuseumEntity | null = await museumRepository.findOne({where: {id: museum.id}, relations: ["artworks"]});
    const deletedArtwork: ArtworkEntity | undefined = storedMuseum?.artworks.find(a => a.id === artwork.id);
  ...
  it('deleteArtworkToMuseum should thrown an exception for an non associated artwork', async () => {
    const newArtwork: ArtworkEntity = await artworkRepository.save({
      name: faker.company.name(), 
      year: faker.number.int({ min: 1900, max: 2025 }),
      description: faker.lorem.sentence(),
